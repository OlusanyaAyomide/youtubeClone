...
export default function Search({bool,toggle}) {
  ...
  return (
    <div className={`flex items-center ${bool?"absolute top-4 w-full inset-0 py-2 pr-2":""}`}>
        <span className={`fa fa-long-arrow-left text-xl mr-2 ${bool?"":"hidden"} font-bold w-1/12 text-center`} onClick={()=>toggle()}></span>
        <input placeholder="Search YouTube" value={input} className={`px-2 h-[36px] text-[18px] outline-none font-semibold text ${!bool?"w-9/12 hidden md:block":" w-10/12"} bg-gray-500/20 rounded-tl-[16px] rounded-bl-[16px] border border-t-black/70 border-r-0 border-l-black/70 border-b-black/70`} onChange={(e)=>{
